...
public partial class Abastecimento_Reenvio : System.Web.UI.Page
{
    string autNome = "";
    string autPlaca = "";
    string autKM = "";
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        string autID = Request.QueryString["v1"];
        DadosAutoriza(autID);
        ...
        string assuntoMensagem = "Re-envio de Autorização de Abastecimento - LOG TRANSPORTES";

        string l1 = "<b><p>RE-ENVIO DA AUTORIZAÇÃO DE ABASTECIMENTO Nº: " + autID + "</p></b>";
        string l2 = "<br/>";
        ...
        try
        {
            ...
            literal_retorno.Text = "Autorização de Abastecimento RE-ENVIADA com sucesso";

        }
        catch (Exception ex)
        {
            literal_retorno.Text = "Ocorreram problemas no envio do e-mail. Erro = " + ex.Message;
        }
        finally
        {
    ...
    public void DadosAutoriza(string IDAut)
    {

        string stringselect = "select Nome , Placa, Kilometragem, Valor from Tbl_Abastecimentos " +
            "where ID_Abastecimento = " + IDAut;
        OperacaoBanco operacao = new OperacaoBanco();
        System.Data.SqlClient.SqlDataReader dados = operacao.Select(stringselect);

        while (dados.Read())
        {
            autNome = Convert.ToString(dados[0]);
            autPlaca = Convert.ToString(dados[1]);
            autKM = Convert.ToString(dados[2]);
            autValor = Convert.ToString(dados[3]);
            break;
        }
